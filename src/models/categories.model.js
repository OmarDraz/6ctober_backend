...
Category.addCategory = (data, file, result) => {
    conn.query("INSERT INTO categories (ar_name, en_name, image) VALUES (?, ?, ?)", [data.ar_name, data.en_name, `${process.env.HOST}/uploads/${file ? file.originalname : 'default.jpg'}`], (err, res) => {
        if(err){
...
Category.updateCategory = (categoryId, data, file, result) => {
    conn.query("UPDATE categories SET ar_name = ?, en_name = ?, image = ? WHERE id = ?", [data.ar_name, data.en_name, `${process.env.HOST}/uploads/${file ? file.originalname : 'default.jpg'}`, +categoryId], (err, res) => {
        if(err){
